...
bool WordIsPalindrome(string word) {
  ...
  // Code: word.at()
  // Logic: word.at(word.length) == word.at(0)
  // Trial: return WordIsPalindrome(word.at(word.length -1))
  // == WordIsPalindrome(word.at(0 + 1))
}
 ...
 * @return a string containing the contents of the array, separated by spaces, 
  in reverse order; returns empty string
 *         if the startIndex is < zero
 */
string ArrayBackwardsAsString(int array[], unsigned int start,
                              unsigned int size) {
